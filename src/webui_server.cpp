...
WebUIServer::WebUIServer(SettingsManager& settingsMgr, MeshNetwork& meshNet, WiFiManager& wifiMgr)
  : settingsManager(settingsMgr), meshNetwork(meshNet), wifiManager(wifiMgr), server(80) {}
...
void WebUIServer::setupRoutes() {
  ...
  server.on("/settings", HTTP_GET, [this](AsyncWebServerRequest *request) {
    ...
    json += "\"overrideR\":" + String(s.overrideR) + ",";
    json += "\"overrideG\":" + String(s.overrideG) + ",";
    json += "\"overrideB\":" + String(s.overrideB) + ",";
  ...
  server.on("/settings", HTTP_POST, [this](AsyncWebServerRequest *request) {
    ...
    if (request->hasParam("overrideR", true))
      settingsManager.getSettings().overrideR = request->getParam("overrideR", true)->value().toInt();
    if (request->hasParam("overrideG", true))
      settingsManager.getSettings().overrideG = request->getParam("overrideG", true)->value().toInt();
    if (request->hasParam("overrideB", true))
      settingsManager.getSettings().overrideB = request->getParam("overrideB", true)->value().toInt();
  ...
  server.on("/wifi", HTTP_POST, [this](AsyncWebServerRequest *request) {
    if (request->hasParam("ssid", true))
      strncpy(settingsManager.getSettings().wifiSSID, request->getParam("ssid", true)->value().c_str(), sizeof(settingsManager.getSettings().wifiSSID) - 1);
    if (request->hasParam("pass", true))
      strncpy(settingsManager.getSettings().wifiPassword, request->getParam("pass", true)->value().c_str(), sizeof(settingsManager.getSettings().wifiPassword) - 1);
    settingsManager.saveSettings();
    request->send(200, "application/json", "{\"status\":\"saved\"}");
    delay(1000);
    ESP.restart();
